...
impl InterAreaResize {
    ...
    pub fn new(new_width: u32, new_height: u32) -> Result<Self, ResizeAreaError> {
    ...
    P: Pixel,
{
    ...
    fn resize_area_mut(&mut self, new_width: u32, new_height: u32) -> Result<&mut Self, ResizeAreaError>;
}
    ...
    P::Subpixel: Clamp<f32> + Into<f32> + Primitive,
{
    ...
    #[doc(hidden)]
    fn resize_area_mut(&mut self, _new_width: u32, _new_height: u32) -> Result<&mut Self, ResizeAreaError> {
        unimplemented!(
...
mod tests {
    ...
    fn test_resize_area_fast() {
        let src = ImageBuffer::from_fn(4, 4, |x, y| {
            Rgb([((x + y) * 50) as u8, 100, 150])
        });
    ...
    fn test_inter_area_resize() {
        let src = ImageBuffer::from_fn(8, 8, |x, y| {
            Rgb([((x + y) * 16) as u8, 100, 150])
        });
