...
impl SatBoxFilter for Image<Luma<f32>> {
    ...
    fn box_filter(&self, x_radius: u32, y_radius: u32) -> Result<Self::Output, Self::Error> {
        ...
        // 積分画像を作成
        let sat = self.create_summed_area_table()
...
impl SatBoxFilter for Image<Luma<u8>> {
    type Output = Image<Luma<f32>>;
    type Error = BoxFilterError;
    ...
    fn box_filter(&self, x_radius: u32, y_radius: u32) -> Result<Self::Output, Self::Error> {
        ...
        // 積分画像を作成
        let sat: SummedAreaTable<u32> = self.create_summed_area_table()
            .map_err(|_| BoxFilterError::EmptyImage)?;
...
impl SatBoxFilter for Image<Rgb<f32>> {
...
impl SatBoxFilter for Image<Rgb<u8>> {
    type Output = Image<Rgb<f32>>;
    type Error = BoxFilterError;
...
fn apply_sat_box_filter_rgb_u8(
    ...
    let r_data: Vec<u32> = image.pixels().map(|p| p[0] as u32).collect();
    let g_data: Vec<u32> = image.pixels().map(|p| p[1] as u32).collect();
    let b_data: Vec<u32> = image.pixels().map(|p| p[2] as u32).collect();
